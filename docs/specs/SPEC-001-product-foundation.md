...
| Regra        | Ancoragem                                                                                                                                                                                                                                                      |
| ------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| RN-01, RN-02 | `Analysis.channelId` é `YouTubeChannelId`; a URL vive em `requestedUrl`, que não é chave. `parseYouTubeChannelReference` (SPEC-002) extrai o identificador e descarta a URL. Testes em `start-channel-analysis.test.ts` e `youtube-channel-reference.test.ts`. |
...
| RN-06        | `ChannelMetrics` tem blocos `shorts` e `long` separados; não existe agregado no nível do canal. A mediana chega por parâmetro a `calculateOutlierScore`, então nenhum caminho de código pode usar a do outro formato (SPEC-003).                               |
| RN-08        | Todo agregado é `number \| null`. `VideoFormat` inclui `'unknown'`. Vídeos sem `viewCount` são excluídos dos agregados em vez de somados como zero, e contados em `videosWithoutViewCount` (SPEC-003).                                                         |
| RN-09        | Estado `partially_completed`; `isReusableStatus` o aceita como válido. `CalculateAnalysisMetrics` termina nele — dados objetivos válidos sem relatório de IA — e reserva `failed` para falha objetiva de cálculo (SPEC-005).                                   |
